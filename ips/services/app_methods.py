...
def edit_run(
    run_id, run_name, run_description, period, year, run_type="0", run_status="0"
):
    """
...
def get_display_data_json(table_name, run_id=None, data_source=None):
    column_sets = {
        "SHIFT_DATA": ["PORTROUTE", "WEEKDAY", "ARRIVEDEPART", "AM_PM_NIGHT", "TOTAL"],
        "TRAFFIC_DATA": [
            "PORTROUTE",
            "PERIODSTART",
            "PERIODEND",
...
def create_process_variables_set(run_id, name, user, period, year):
    log.debug(
        f"app_methods: create_process_variables_set: {run_id}, {name}, {user}, {period}, {year}"
    )
...
def get_process_variable_sets():
    ...
    response = requests.get(API_TARGET + r"/pv_sets")
    log.debug(f"get_process_variable_sets response content: {response.content}")
    log.debug(f"get_process_variable_sets response status: {response.status_code}")
...
def edit_single_process_variable(run_id, pv_name, pv_json):
    ...
    return requests.post(
        API_TARGET + r"/single_process_variable/" + run_id, json=pv_json
    )
...
def survey_data_import(import_data_file, month, year):
    # Import  data
    log.debug(
        f"app_methods: survey_data_import: data_file: {import_data_file},  month: {month}, year: {year}"
    )
...
def get_run_step_requests(run_id, step_number=None):
    log.debug(
        f"app_methods: get_run_step_requests: run_id: {run_id},  step_number: {step_number}"
    )
